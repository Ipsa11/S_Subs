...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("Saita"),
	impl_name: create_runtime_str!("Saita-stage"),
	authoring_version: 0,
	spec_version: 100,
	impl_version: 0,
...
pub struct ToBurn;
impl OnUnbalanced<NegativeImbalance> for ToBurn {
	fn on_nonzero_unbalanced(amount: NegativeImbalance) {
		if let Some(author) = Burn::burn_account() {
...
parameter_types! {
	pub const TransactionByteFee: Balance = 11 * MILLI;
...
impl FeeCalculator for FixedGasPrice {
	fn min_gas_price() -> (U256, Weight) {
		 (1_910_000_000_000_000u128.into(), Weight::from_parts(0u64,0))
...
where R: pallet_authorship::Config + pallet_reward::Config,  
{
	fn on_nonzero_unbalanced(amount: NegativeImbalance) {
		let split = amount.ration(87, 13);

		let author = pallet_authorship::Pallet::<R>::author().unwrap();
		let count = pallet_reward::AuthorBlockList::<R>::get(&author).unwrap_or(0);
		let new_count = count + 1;
		let transaction = pallet_reward::TotalReward::<R>::get().unwrap_or(0);
		let new_transaction = transaction + 1;
		pallet_reward::TotalReward::<R>::put(new_transaction);
		pallet_reward::AuthorBlockList::<R>::insert(author, new_count);


		Treasury::on_unbalanced(split.0);
		ToBurn::on_unbalanced(split.1);
	}
}
...
impl pallet_burn::Config for Runtime{
	...
	type BurnOrigin =  EitherOf<EnsureRoot<AccountId>, GeneralAdmin>;
...
construct_runtime! {
	pub enum Runtime 
	{
		...
		Burn:pallet_burn::{Pallet, Storage,Call, Config<T>, Event<T>} = 27,
