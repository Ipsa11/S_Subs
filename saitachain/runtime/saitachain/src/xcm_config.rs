...
//! XCM configuration for SaitaChain.
...
use xcm_builder::{
	...
	OriginToPluralityVoice, SignedAccountId32AsNative,
...
parameter_types! {
	/// The location of the STC token, from the context of this chain. Since this token is native to this
	/// chain, we make it synonymous with it and thus it is the `Here` location, which means "equivalent to
	/// the context".
	pub const TokenLocation: MultiLocation = Here.into_location();
	/// The SaitaChain network ID. This is named.
	pub const ThisNetwork: NetworkId = NetworkId::SaitaChain;
...
pub type SovereignAccountOf = (
	...
	// We can directly alias an `AccountId32` into a local account.
	 xcm_builder::AccountKey20Aliases<ThisNetwork, AccountId>,
);
...
parameter_types! {
	pub const Stc: MultiAssetFilter = Wild(AllOf { fun: WildFungible, id: Concrete(TokenLocation::get()) });
	pub const StatemintLocation: MultiLocation = Parachain(STATEMINT_ID).into_location();
	pub const DotForStatemint: (MultiAssetFilter, MultiLocation) = (Stc::get(), StatemintLocation::get());
	pub const CollectivesLocation: MultiLocation = Parachain(COLLECTIVES_ID).into_location();
	pub const DotForCollectives: (MultiAssetFilter, MultiLocation) = (Stc::get(), CollectivesLocation::get());
	pub const MaxAssetsIntoHolding: u32 = 64;
}

/// Saitama Relay recognizes/respects the Statemint chain as a teleporter.
...
impl xcm_executor::Config for XcmConfig {
	...
	// Saitama Relay recognises no chains which act as reserves.
	type IsReserve = ();
	type IsTeleporter = TrustedTeleporters;
	type UniversalLocation = UniversalLocation;
	type Barrier = Barrier;
	type Weigher = WeightInfoBounds<
		crate::weights::xcm::SAITACHAINXcmWeight<RuntimeCall>,
		RuntimeCall,
		MaxInstructions,
	>;
...
pub type LocalOriginToLocation = (
	...
	xcm_builder::SignedToAccountId20<RuntimeOrigin, AccountId, ThisNetwork>,
);
