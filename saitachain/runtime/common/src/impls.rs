...
//! Auxiliary `struct`/`enum`s for saitachain runtime.

use crate::NegativeImbalance;
use frame_support::traits::{Currency, Imbalance, OnUnbalanced};
use primitives::Balance;
use sp_runtime::Perquintill;
use pallet_reward::AuthorBlockList;
use pallet_reward::TotalReward;
/// Logic for the author to get a portion of fees.
pub struct ToAuthor<R>(sp_std::marker::PhantomData<R>);
impl<R> OnUnbalanced<NegativeImbalance<R>> for ToAuthor<R>
where
	R: pallet_balances::Config + pallet_authorship::Config + pallet_reward::Config ,
	...
	<R as frame_system::Config>::AccountId: Into<primitives::AccountId>,
{
	fn on_nonzero_unbalanced(amount: NegativeImbalance<R>) {
		if let Some(author) = <pallet_authorship::Pallet<R>>::author() {
			<pallet_balances::Pallet<R>>::resolve_creating(&author.clone(), amount);
		}
	}
}

pub struct ToBurn<R>(sp_std::marker::PhantomData<R>);
impl<R> OnUnbalanced<NegativeImbalance<R>>
	for ToBurn<R>
	where
		R: pallet_balances::Config + pallet_authorship::Config + pallet_burn::Config,
		<R as frame_system::Config>::AccountId: From<primitives::AccountId>,
		<R as frame_system::Config>::AccountId: Into<primitives::AccountId>
{
	fn on_nonzero_unbalanced(amount: NegativeImbalance<R>) {
		if let Some(account) = <pallet_burn::Pallet<R>>::burn_account() {
...
impl<R> OnUnbalanced<NegativeImbalance<R>>
	for DealWithFees<R>
	where
		R: pallet_balances::Config + pallet_treasury::Config + pallet_authorship::Config +	pallet_reward::Config + pallet_burn::Config ,
		...
		
{
	fn on_unbalanceds<B>(mut fees_then_tips: impl Iterator<Item = NegativeImbalance<R>>) {
		if let Some(fees) = fees_then_tips.next() {
			...
			let mut split = fees.ration(87, 13);

			if let Some(tips) = fees_then_tips.next() {
				tips.merge_into(&mut split.1);
			}

			let author = pallet_authorship::Pallet::<R>::author().unwrap();
			let count = AuthorBlockList::<R>::get(&author).unwrap_or(0);
			let new_count = count + 1;
			let transaction = TotalReward::<R>::get().unwrap_or(0);
			let new_transaction = transaction + 1;
			TotalReward::<R>::put(new_transaction);
			AuthorBlockList::<R>::insert(author, new_count);

		
			use pallet_treasury::Pallet as Treasury;
			<Treasury<R> as OnUnbalanced<_>>::on_unbalanced(split.0);
			<ToBurn<R> as OnUnbalanced<_>>::on_unbalanced(split.1);
