...
//! Overview over request/responses as used in `SaitaChain`.
//!
...
impl Protocol {
	...
	fn create_config(
		...
		match self {
			...
			Protocol::StatementFetchingV1 => RequestResponseConfig {
				...
				// - which is the recommended minimum bandwidth for nodes  as of April
				// 2021.
	...
	const fn get_legacy_name(self) -> Option<&'static str> {
		match self {
			Protocol::ChunkFetchingV1 => Some("/saitachain/req_chunk/1"),
			Protocol::CollationFetchingV1 => Some("/saitachain/req_collation/1"),
			Protocol::PoVFetchingV1 => Some("/saitachain/req_pov/1"),
			Protocol::AvailableDataFetchingV1 => Some("/saitachain/req_available_data/1"),
			Protocol::StatementFetchingV1 => Some("/saitachain/req_statement/1"),
			Protocol::DisputeSendingV1 => Some("/saitachain/send_dispute/1"),
