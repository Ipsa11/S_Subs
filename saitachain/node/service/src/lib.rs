...
//! SaitaChain service. Specialized wrapper over substrate service.
...
use {
	grandpa::{self},
	gum::info,
	saitama_node_core_approval_voting::{
		self as approval_voting_subsystem, Config as ApprovalVotingConfig,
	},
	saitama_node_core_av_store::Config as AvailabilityConfig,
	saitama_node_core_av_store::Error as AvailabilityError,
	saitama_node_core_candidate_validation::Config as CandidateValidationConfig,
	saitama_node_core_chain_selection::{
		self as chain_selection_subsystem, Config as ChainSelectionConfig,
	},
	saitama_node_core_dispute_coordinator::Config as DisputeCoordinatorConfig,
	saitama_node_network_protocol::{
...
use saitama_node_subsystem_util::database::Database;

#[cfg(feature = "full-node")]
pub use {
	saitama_overseer::{Handle, Overseer, OverseerConnector, OverseerHandle},
	saitama_primitives::runtime_api::ParachainHost,
...
//     	saitachain::api::dispatch(method, data)
//     }
//     fn native_version() -> sc_executor::NativeVersion {
//       	saitachain::native_version()
//     }
// }
...
use saitama_node_subsystem::jaeger;
...
pub use saitama_primitives::{Block, BlockId, BlockNumber, CollatorPair, Hash, Id as ParaId};
...
pub use {saitama_runtime, saitama_runtime_constants};
...
pub enum Error {
	...
	Overseer(#[from] saitama_overseer::SubsystemError),
	...
	#[error(transparent)]
	Jaeger(#[from] saitama_node_subsystem::jaeger::JaegerError),
	...
	#[cfg(feature = "full-node")]
	#[error("Expected at least one of saitachain runtime feature")]
	NoRuntime,
	...
	#[cfg(feature = "full-node")]
	#[error("Worker binaries could not be found, make sure saitachain was built/installed correctly. If you ran with `cargo run`, please run `cargo build` first. Searched given workers path ({given_workers_path:?}), saitachain binary path ({current_exe_path:?}), and lib path (/usr/lib/saitachain), workers names: {workers_names:?}")]
...
pub trait IdentifyVariant {
	/// Returns if this is a configuration for the `SaitaChain` network.
	fn is_saitachain(&self) -> bool;
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_saitachain(&self) -> bool {
		self.id().starts_with("saitachain") || self.id().starts_with("stc")
	}
...
pub fn open_database(db_source: &DatabaseSource) -> Result<Arc<dyn Database>, Error> {
	let parachains_db = match db_source {
		...
		DatabaseSource::Custom { .. } => {
			unimplemented!("No saitachain subsystem db for custom source.");
		},
	};
	Ok(parachains_db)
}
	...
	ChainSelection: 'static + SelectChain<Block>,
{
	...
	let overrides = saitachain_rpc::overrides_handle(client.clone());
	let eth: saitachain_rpc::EthConfiguration = Default::default();
	let frontier_backend = match eth.frontier_backend_type {
		saitachain_rpc::BackendType::KeyValue => FrontierBackend::KeyValue(fc_db::kv::Backend::open(
			Arc::clone(&client),
			&config.database,
			&db_config_dir(config),
		)?),
		saitachain_rpc::BackendType::Sql => {
	...
	let _warp_sync = Arc::new(grandpa::warp_proof::NetworkProvider::new(
		backend.clone(),
...
/// If not specified, the main binary's directory is searched first, then `/usr/lib/saitachain` is
...
pub fn new_full<OverseerGenerator: OverseerGen>(
	...
	use saitama_node_network_protocol::request_response::IncomingRequest;
	...
	// Warn the user that BEEFY is still experimental for SaitaChain.
	if enable_beefy && config.chain_spec.is_saitachain() {
		gum::warn!("BEEFY is still experimental, usage on SaitaChain network is discouraged.");
	}
	...
	let select_chain = if auth_or_collator {
		let metrics =
			saitama_node_subsystem_util::metrics::Metrics::register(prometheus_registry.as_ref())?;
	...
	// Note: GrandPa is pushed before the SaitaChain-specific protocols. This doesn't change
	...
	if role.is_authority() || is_parachain_node.is_running_alongside_parachain_node() {
		use saitama_network_bridge::{peer_sets_info, IsAuthority};
	...
	let rpc_extensions_builder = {
		...
		Box::new(move |deny_unsafe,  subscription_executor: saitachain_rpc::SubscriptionTaskExecutor|
			{
			let deps = saitachain_rpc::FullDeps {
				client: client.clone(),
				...
				babe:saitachain_rpc::BabeDeps {
					keystore: keystore.clone(),
					babe_worker_handle: babe_worker_handle.clone(),
				},
				grandpa: saitachain_rpc::GrandpaDeps {
				...
				beefy:saitachain_rpc::BeefyDeps {
			...
			saitachain_rpc::create_full::<service::client::Client<sc_client_db::Backend<Block>, LocalCallExecutor
			<Block, 
			sc_client_db::Backend<Block>, WasmExecutor<((sp_io::storage::HostFunctions, sp_io::default_child_storage::HostFunctions, 
				...
				sp_io::panic_handler::HostFunctions, sp_io::logging::HostFunctions, sp_io::trie::HostFunctions,
			sp_io::offchain_index::HostFunctions, sp_io::transaction_index::HostFunctions), 
			 frame_benchmarking::benchmarking::HostFunctions)>>,  Block, fake_runtime_api::RuntimeApi>, Block>, Block>, SelectRelayChain<sc_client_db::Backend<Block>>, sc_client_db::Backend<Block>, 
...
) 
{
	...
	if let Some(hwbench) = hwbench {
		...
		if !SUBSTRATE_REFERENCE_HARDWARE.check_hardware(&hwbench) && role.is_authority() {
			log::warn!(
				...
				https://wiki.saitachain.network/docs/maintain-guides-how-to-validate-saitachain#reference-hardware"
			);
		}
...

		{
			...
			task_manager.spawn_essential_handle().spawn_blocking(
				...
				Box::pin(async move {
					...
					let forward = saitama_overseer::forward_events(overseer_client, handle);
	...
	if role.is_authority() {
		...
		let babe_config = babe::BabeParams {
			...
			create_inherent_data_providers: move |parent, ()| {
				...
				async move {
					let parachain =
						saitama_node_core_parachains_inherent::ParachainsInherentDataProvider::new(
							client_clone,
							overseer_handle,
							parent,
						);
	...
	let config = grandpa::Config {
		...
		// https://github.com/paritytech/saitachain/issues/5464
...
/// Build a full node.
///
/// The actual "flavor", aka if it will use `SaitaChain` is determined based on
...
pub fn build_full<OverseerGenerator: OverseerGen>(
	...
	let is_saitachain = config.chain_spec.is_saitachain();

	params.overseer_message_channel_capacity_override =
		params.overseer_message_channel_capacity_override.map(move |capacity| {
			if is_saitachain {
				gum::warn!("Channel capacity should _never_ be tampered with on saitachain!");
			}
			capacity
		});
