...
use saitama_primitives::AccountId;
	...
	(
		...
		match $chain {
			Chain::SaitaChain => {
				...
				{
					use saitama_runtime as runtime;

					let call = $generic_code;

					Ok(saitachain_sign_call(call, $nonce, $current_block, $period, $genesis, $signer))
				}

				#[cfg(not(feature = "saitachain-native"))]
				{
					Err("`saitachain-native` feature not enabled")
				}
			},
...
impl frame_benchmarking_cli::ExtrinsicBuilder for RemarkBuilder {
	...
	fn build(&self, nonce: u32) -> std::result::Result<OpaqueExtrinsic, &'static str> {
		let period = saitama_runtime_common::BlockHashCount::get()
			.checked_next_power_of_two()
			.map(|c| c / 2)
			.unwrap_or(2) as u64;
		let genesis = self.client.usage_info().chain.best_hash;
		let signer =  sp_core::ecdsa::Pair::from_string("//Bob", None).expect("static values are valid; qed");
		let current_block = 0;
...
#[cfg(feature = "saitachain-native")]
fn saitachain_sign_call(
	call: saitama_runtime::RuntimeCall,
	nonce: u32,
	current_block: u64,
	period: u64,
	genesis: sp_core::H256,
	acc: sp_core::ecdsa::Pair,
) -> OpaqueExtrinsic {
	use codec::Encode;
	use saitama_runtime as runtime;
	...
	let extra: runtime::SignedExtra = (
		...
		saitama_runtime_common::claims::PrevalidateAttests::<runtime::Runtime>::new(),
	);
	...
	runtime::UncheckedExtrinsic::new_signed(
		call,
		fp_account::AccountId20::from(acc.public()).into(),
		fp_account::EthereumSignature(signature.clone()),
		extra,
	)
	.into()
}
...
fn westend_sign_call(
	...
	runtime::UncheckedExtrinsic::new_signed(
		...
		saitama_core_primitives::Signature::Sr25519(signature.clone()),
		extra,
	)
	.into()
}

/// Generates inherent data for benchmarking SaitaChain
///
/// Not to be used outside of benchmarking since it returns mocked values.
pub fn benchmark_inherent_data(
	header: saitama_core_primitives::Header,
	...
	let para_data = saitama_primitives::InherentData {
		bitfields: Vec::new(),
	...
	inherent_data.put_data(saitama_primitives::PARACHAINS_INHERENT_IDENTIFIER, &para_data)?;
