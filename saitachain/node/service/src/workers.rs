...
/// Override the workers saitachain binary directory path, used for testing.
#[cfg(test)]
...
fn list_workers_paths(
	...
	// Consider the saitachain binary directory.
	{
	...
	// Consider the /usr/lib/saitachain/ directory.
	{
		#[allow(unused_mut)]
		let mut lib_path = PathBuf::from("/usr/lib/saitachain");
		#[cfg(test)]
		...
		let (prep_worker_exists, exec_worker_exists) = (prep_worker.exists(), exec_worker.exists());
		if prep_worker_exists && exec_worker_exists {
			log::trace!("Worker binaries found at /usr/lib/saitachain");
			workers_paths.push((prep_worker, exec_worker));
		} else if prep_worker_exists {
			log::warn!("Worker binary found at {:?} but not {:?}", prep_worker, exec_worker);
		} else if exec_worker_exists {
...
fn build_worker_paths(
	...
	let (prep_worker_name, exec_worker_name) = workers_names.unwrap_or((
		saitama_node_core_pvf::PREPARE_BINARY_NAME.to_string(),
		saitama_node_core_pvf::EXECUTE_BINARY_NAME.to_string(),
	));
...
",
	...
	fn with_temp_dir_structure(
		...
		// Set up <tmp>/usr/lib/saitachain and <tmp>/usr/bin, both empty.

		let tempdir = temp_dir();
		let lib_path = tempdir.join("usr/lib/saitachain");
	...
	fn test_given_worker_path() {
		with_temp_dir_structure(|tempdir, exe_path| {
			...
			let prepare_worker_path = given_workers_path.join("SaitaChain-prepare-worker");
			write_worker_exe(&prepare_worker_path)?;
			fs::set_permissions(&prepare_worker_path, fs::Permissions::from_mode(0o644))?;
			let execute_worker_path = given_workers_path.join("SaitaChain-execute-worker");
	...
	fn missing_workers_paths_throws_error() {
		with_temp_dir_structure(|tempdir, exe_path| {
			...
			let prepare_worker_path = tempdir.join("usr/bin/SaitaChain-prepare-worker");
			write_worker_exe(&prepare_worker_path)?;
			assert_matches!(
				determine_workers_paths(None, None, Some(NODE_VERSION.into())),
				Err(Error::MissingWorkerBinaries { given_workers_path: None, current_exe_path: p, workers_names: None }) if p == exe_path
			...
			let execute_worker_path = tempdir.join("usr/bin/SaitaChain-execute-worker");
			write_worker_exe(&execute_worker_path)?;
			assert_matches!(
				determine_workers_paths(None, None, Some(NODE_VERSION.into())),
				Err(Error::MissingWorkerBinaries { given_workers_path: None, current_exe_path: p, workers_names: None }) if p == exe_path
			...
			let prepare_worker_path = tempdir.join("usr/lib/saitachain/SaitaChain-prepare-worker");
			write_worker_exe(&prepare_worker_path)?;
			assert_matches!(
				determine_workers_paths(None, None, Some(NODE_VERSION.into())),
				Err(Error::MissingWorkerBinaries { given_workers_path: None, current_exe_path: p, workers_names: None }) if p == exe_path
			...
			fs::remove_file(&prepare_worker_path)?;
			let execute_worker_path = tempdir.join("usr/lib/saitachain/SaitaChain-execute-worker");
			write_worker_exe(execute_worker_path)?;
			assert_matches!(
				determine_workers_paths(None, None, Some(NODE_VERSION.into())),
				Err(Error::MissingWorkerBinaries { given_workers_path: None, current_exe_path: p, workers_names: None }) if p == exe_path
	...
	fn should_find_workers_at_all_locations() {
		with_temp_dir_structure(|tempdir, _| {
				let prepare_worker_bin_path = tempdir.join("usr/bin/SaitaChain-prepare-worker");
				write_worker_exe(&prepare_worker_bin_path)?;

				let execute_worker_bin_path = tempdir.join("usr/bin/SaitaChain-execute-worker");
				write_worker_exe(&execute_worker_bin_path)?;

				let prepare_worker_lib_path = tempdir.join("usr/lib/saitachain/SaitaChain-prepare-worker");
				write_worker_exe(&prepare_worker_lib_path)?;

				let execute_worker_lib_path = tempdir.join("usr/lib/saitachain/SaitaChain-execute-worker");
	...
	fn should_find_workers_with_custom_names_at_all_locations() {
		with_temp_dir_structure(|tempdir, _| {
			...
			let prepare_worker_lib_path = tempdir.join("usr/lib/saitachain").join(prep_worker_name);
			write_worker_exe(&prepare_worker_lib_path)?;

			let execute_worker_lib_path = tempdir.join("usr/lib/saitachain").join(exec_worker_name);
			write_worker_exe(&execute_worker_lib_path)?;
