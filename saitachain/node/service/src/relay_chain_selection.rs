...
use saitama_node_primitives::MAX_FINALITY_LAG as PRIMITIVES_MAX_FINALITY_LAG;
use saitama_node_subsystem::messages::{
...
use saitama_node_subsystem_util::metrics::{self, prometheus};
use saitama_overseer::{AllMessages, Handle};
use saitama_primitives::{Block as SaitaChainBlock, BlockNumber, Hash, Header as SaitaChainHeader};
use std::sync::Arc;
...
const MAX_FINALITY_LAG: saitama_primitives::BlockNumber = PRIMITIVES_MAX_FINALITY_LAG;
		...
		let metrics = MetricsInner {
			approval_checking_finality_lag: prometheus::register(
				prometheus::Gauge::with_opts(
					prometheus::Opts::new(
						"saitachain_parachain_approval_checking_finality_lag",
			...
			disputes_finality_lag: prometheus::register(
				prometheus::Gauge::with_opts(
					prometheus::Opts::new(
						"saitachain_parachain_disputes_finality_lag",
...
/// into account.
enum IsDisputesAwareWithOverseer<B: sc_client_api::Backend<SaitaChainBlock>> {
...
where
	B: sc_client_api::Backend<SaitaChainBlock>,
...
pub struct SelectRelayChain<B: sc_client_api::Backend<SaitaChainBlock>> {
	longest_chain: sc_consensus::LongestChain<B, SaitaChainBlock>,
...
where
	B: sc_client_api::Backend<SaitaChainBlock> + 'static,
{
	...
	pub fn as_longest_chain(&self) -> &sc_consensus::LongestChain<B, SaitaChainBlock> {
		&self.longest_chain
	}
}

#[async_trait::async_trait]
impl<B> SelectChain<SaitaChainBlock> for SelectRelayChain<B>
where
	B: sc_client_api::Backend<SaitaChainBlock> + 'static,
{
	...
	async fn best_chain(&self) -> Result<SaitaChainHeader, ConsensusError> {
		match self.selection {
...
where
	B: HeaderProviderProvider<SaitaChainBlock>,
	OH: OverseerHandleT,
{
	...
	fn block_header(&self, hash: Hash) -> Result<SaitaChainHeader, ConsensusError> {
...
where
	B: HeaderProviderProvider<SaitaChainBlock> + Send + Sync,
	OH: OverseerHandleT,
{
	fn clone(&self) -> Self {
