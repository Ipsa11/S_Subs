...
//! # SaitaChain Staking Miner.
//!
//! Simple bot capable of monitoring a saitachain (and cousins) chain and submitting solutions to the
...
pub(crate) static mut RUNTIME: AnyRuntime = AnyRuntime::SaitaChain;
...
fn signed_ext_builder_saitachain(
	nonce: Nonce,
	tip: Balance,
	era: sp_runtime::generic::Era,
) -> saitama_runtime_exports::SignedExtra {
	use saitama_runtime_exports::Runtime;
	(
...
construct_runtime_prelude!(saitachain);
...
macro_rules! any_runtime {
	($($code:tt)*) => {
		unsafe {
			match $crate::RUNTIME {
				$crate::AnyRuntime::SaitaChain => {
					#[allow(unused)]
					use $crate::saitama_runtime_exports::*;
					$($code)*
				},
			}
		}
