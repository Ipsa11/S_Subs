...
use saitachain_parachain_primitives::primitives::Id as ParaId;
use saitama_runtime_parachains::{configuration, origin, shared};
...
parameter_types! {
	pub const StcLocation: MultiLocation = MultiLocation::here();
	pub const SaitaNetwork: NetworkId = NetworkId::SaitaChain;
	pub UniversalLocation: InteriorMultiLocation = Here;
	pub CheckAccount: (AccountId, MintLocation) = (XcmPallet::check_account(), MintLocation::Local);
}

pub type SovereignAccountOf =
	(ChildParachainConvertsVia<ParaId, AccountId>, AccountId32Aliases<SaitaNetwork, AccountId>);
...
type LocalOriginConverter = (
	...
	SignedAccountId32AsNative<SaitaNetwork, RuntimeOrigin>,
...
parameter_types! {
	pub SaitaChainForStatemine: (MultiAssetFilter, MultiLocation) =
		(Wild(AllOf { id: Concrete(Here.into()), fun: WildFungible }), Parachain(1000).into());
	pub const MaxInstructions: u32 = 100;
	pub const MaxAssetsIntoHolding: u32 = 4;
}

pub type TrustedTeleporters = (xcm_builder::Case<SaitaChainForStatemine>,);
...
pub type LocalOriginToLocation = SignedToAccountId32<RuntimeOrigin, AccountId, SaitaNetwork>;
...
pub fn saitachain_like_with_balances(balances: Vec<(AccountId, Balance)>) -> sp_io::TestExternalities {
