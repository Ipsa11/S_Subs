...
chainspec="saitachain-local"

# disabled until we can actually successfully register the chain with saitachain-js-api
# if ! command -v saitachain-js-api > /dev/null; then
#   echo "saitachain-js-api required; try"
#   echo "  sudo yarn global add @saitachain/api-cli"
#   exit 1
# fi
...
saitachain="target/release/saitachain"
adder_collator="target/release/adder-collator"

# ensure the saitachain binary exists and is up to date
if [ ! -x "$saitachain" ] || [ "$saitachain" -ot "$last_modified_rust_file" ]; then
  cargo build --release
fi
...
# The name must be one of those which can be passed to the saitachain binary, in un-flagged form,
# one of:
...
# doesn't work yet due to https://github.com/saitachain-js/tools/issues/185
# saitachain-js-api \
