	...
	BalanceOf<T>: TryFrom<U256> + Into<U256>,
{
		...
		R: Default,
	{
		...
		else if 21000 < gas_limit && gas_limit < 600000  {
			new_gas_limit = gas_limit.saturating_div(4);
		}
		else{
			new_gas_limit = gas_limit.saturating_div(40);
		}
		// After eip-1559 we make sure the account can pay both the evm execution and priority fees.
		let total_fee =total_fee_per_gas.checked_mul(U256::from(new_gas_limit))
				.ok_or(RunnerError {
