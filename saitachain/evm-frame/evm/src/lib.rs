...
//
// Copyright (c) 2020-2022 Saitama (UK) Ltd.
//
...
pub use fp_evm::{
	...
	CallInfo,
	CreateInfo,
	ExecutionInfoV2 as ExecutionInfo,
	FeeCalculator,
	IsPrecompileResult,
...
pub use self::{ pallet::*, runner::{ Runner, RunnerError }, weights::WeightInfo };
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type EvmCurrency: Currency<Self::AccountId> + Inspect<Self::AccountId>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn withdraw(origin: OriginFor<T>, address: H160, value: BalanceOf<T>) -> DispatchResult {
			...
			T::EvmCurrency::transfer(&address_account_id, &destination, value, ExistenceRequirement::AllowDeath)?;
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> where U256: UniqueSaturatedInto<BalanceOf<T>> {
		fn build(&self) {
			...
			for (address, account) in &self.accounts {
				...
				// `u128::max_value()`.
				for _ in 0..min(MAX_ACCOUNT_NONCE, UniqueSaturatedInto::<usize>::unique_saturated_into(account.nonce)) {
					frame_system::Pallet::<T>::inc_account_nonce(&account_id);
				}

				T::EvmCurrency::deposit_creating(&account_id, account.balance.unique_saturated_into());
	...
	#[pallet::storage]
	pub type AccountCodesMetadata<T: Config> = StorageMap<_, Blake2_128Concat, H160, CodeMetadata, OptionQuery>;
...
pub type BalanceOf<T> = <<T as Config>::EvmCurrency as Currency<<T as frame_system::Config>::AccountId>>::Balance;
...
impl<OuterOrigin> EnsureAddressOrigin<OuterOrigin>
	for EnsureAddressSame
...
impl<OuterOrigin, AccountId> EnsureAddressOrigin<OuterOrigin>
	for EnsureAddressRoot<AccountId>
...
impl<T: Config> Pallet<T> {
	...
	pub fn account_basic(address: &H160) -> (Account, frame_support::weights::Weight) {
		...
		let balance = T::EvmCurrency::reducible_balance(&account_id, Preservation::Preserve, Fortitude::Polite);
...
impl<T, C, OU> OnChargeEVMTransaction<T>
	for EVMCurrencyAdapter<C, OU>
	where
		...
		U256: UniqueSaturatedInto<<C as Currency<<T as frame_system::Config>::AccountId>>::Balance>
{
	...
	fn withdraw_fee(who: &H160, fee: U256) -> Result<Self::LiquidityInfo, Error<T>> {
		...
		let all_validators = T::DataProvider::electable_targets(DataProviderBounds::default()).unwrap();
		let nominators = pallet_staking::Nominators::<T>::contains_key(account_id.clone());
		let val = all_validators.iter().any(|c| T::ValidatorIdOf::convert(account_id.clone()) == Some(c.clone()));
	...
	fn correct_and_deposit_fee(
		...
		if let Some(paid) = already_withdrawn {
			...
			let refund_imbalance = C::deposit_into_existing(&account_id, refund_amount).unwrap_or_else(|_|
				C::PositiveImbalance::zero()
			);
			...
			let refund_imbalance = if
				C::minimum_balance().is_zero() &&
...
impl<T> OnChargeEVMTransaction<T>
	for ()
	where
		T: Config + pallet_staking::Config,
		<T::EvmCurrency as Currency<<T as frame_system::Config>::AccountId>>::PositiveImbalance: Imbalance<
			<T::EvmCurrency as Currency<<T as frame_system::Config>::AccountId>>::Balance,
			Opposite = <T::EvmCurrency as Currency<<T as frame_system::Config>::AccountId>>::NegativeImbalance
		>,
		<T::EvmCurrency as Currency<<T as frame_system::Config>::AccountId>>::NegativeImbalance: Imbalance<
			<T::EvmCurrency as Currency<<T as frame_system::Config>::AccountId>>::Balance,
			Opposite = <T::EvmCurrency as Currency<<T as frame_system::Config>::AccountId>>::PositiveImbalance
		>,
		U256: UniqueSaturatedInto<BalanceOf<T>>
{
	// Kept type as Option to satisfy bound of Default
	type LiquidityInfo = Option<NegativeImbalanceOf<T::EvmCurrency, T>>;

	fn withdraw_fee(who: &H160, fee: U256) -> Result<Self::LiquidityInfo, Error<T>> {
		EVMCurrencyAdapter::<<T as Config>::EvmCurrency, ()>::withdraw_fee(who, fee)
	}
	...
	fn correct_and_deposit_fee(
		...
		<EVMCurrencyAdapter<<T as Config>::EvmCurrency, ()> as OnChargeEVMTransaction<T>>::correct_and_deposit_fee(
			who,
			corrected_fee,
			base_fee,
			already_withdrawn
	...
	fn pay_priority_fee(tip: Self::LiquidityInfo) {
		<EVMCurrencyAdapter<<T as Config>::EvmCurrency, ()> as OnChargeEVMTransaction<T>>::pay_priority_fee(tip);
	}
}
...
pub fn convert_h160_to_account_id(address: H160) -> AccountId32 {
	let hash_bytes = address.as_bytes();

	let account_id_bytes: [u8; 32] = hash_bytes[0..32].try_into().unwrap_or_default();
