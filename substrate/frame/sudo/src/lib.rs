...
pub trait SudoAccountId<AccountId> {
	fn sudo_account() -> AccountId;
}

impl<T:Config> SudoAccountId<T::AccountId> for Pallet<T>{
    fn sudo_account() -> T::AccountId {
        let a = Key::<T>::get().unwrap();
		a
    }
}