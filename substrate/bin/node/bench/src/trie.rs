...
lazy_static! {
	static ref SAITA_STATE_DISTRIBUTION: SizePool =
		SizePool::from_histogram(crate::state_sizes::SAITA_STATE_DISTRIBUTION);
}
...
impl core::BenchmarkDescription for TrieReadBenchmarkDescription {
	...
	fn setup(self: Box<Self>) -> Box<dyn core::Benchmark> {
		...
		let warmup_prefix = SAITA_STATE_DISTRIBUTION.key(&mut rng);
		...
		for idx in 0..self.database_size.keys() {
			let kv = (
				SAITA_STATE_DISTRIBUTION.key(&mut rng).to_vec(),
				SAITA_STATE_DISTRIBUTION.value(&mut rng),
			);
